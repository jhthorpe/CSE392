...
int Forces::LJ_omp_bound(int *N, double *sl, double *sig, double *eps, vector<double> *pos, vector<double> *force)
{
  ...
  #pragma omp parallel 
  {
    ...
    #pragma omp for simd schedule(dynamic,8) private(j,f,r,rx,ry,rz) nowait
      for (i=0; i < *N; i++)
      {
        for (j=i+1; j < *N; j++)
        { 
...
int Forces::elc_pvfmm(int *N, double *sl, vector<double> *q, vector<double> *pos, vector<double> *force)
{
  // Variables
  // N		: number of particles (int)
  // sl		: length of cube (nm, int)
