...
void verlet:: Integration(int *N, int *nsteps, double *tstep, double *sl, double *boxL, double *sig, double *eps, vector<double> *m, vector<double> *pos, vector<double> *vel) {
...
do {
  ...
  forces.LJ_seq(&N,&sl,&sig,&eps,&pos,&f1);      //Calculate force on all N atoms at t=tstep*step
  ...
  forces.LJ_seq(&N,&sl,&sig,&eps,&pos,&f2);      //Calculate force on all N atoms after dt   
