...
void verlet:: Integration(int *N, int *nsteps, int *showsteps, double *tstep, double *sl, double *sig, double *eps, vector<double> *q, vector<double> *m, vector<double> *pos, vector<double> *vel, MPI_Comm* comm) {
...
do {
  ...
  pot.elc_pvfmm(N,sl,q,pos,&f1,&elcpot,comm);      //Calculate elc forces and potnetial energy on all N atoms at t=tstep*step
  ...
  pot.elc_pvfmm(N,sl,q,pos,&f2,&elcpot,comm);         //Calculate elc force on all N atoms after dt
